...
#[derive(Config)]
struct TestDimsConfig {
    batch_size: usize,
    d_inner: usize,
    d_state: usize,
}

fn test_tensor_operations<B: Backend>(device: &B::Device) {
    let batch_size = 2;
    let d_inner = 3;
    let d_state = 4;

    // Test sum_dim output shape
    let h = Tensor::<B, 3>::zeros([batch_size, d_inner, d_state], device);
    let c_expanded = Tensor::<B, 3>::ones([batch_size, d_inner, d_state], device);
    let y_from_h = (h * c_expanded).sum_dim(2);
    println!("y_from_h shape after sum_dim: {:?}", y_from_h.dims());

    // Test element-wise multiplication
    let x_t = Tensor::<B, 2>::ones([batch_size, d_inner], device);
    let d = Tensor::<B, 1>::ones([d_inner], device);
...
fn main() {
    ...
    test_tensor_operations::<NdArray>(&device);
}